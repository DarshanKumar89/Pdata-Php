...
//     $paramName = $req->params('phone'); // Getting parameter with phone
    
 
//    $sql = "INSERT INTO contact (`phone`) VALUES (:phone)";
//   try {
//     $db = getDB();
//     $stmt = $db->prepare($sql);
//      // $stmt->bindParam("phone", $body->phone);
//       $stmt->bindParam("phone", $paramName);
...
// }
//put
$app->put('/update/:contact_id', 'updateContact');
function updateContact($contact_id) {
 ...
 $request  = $app->request();
    ...
    $w = json_decode($body);
    $sql = "UPDATE contact SET first_name=:first_name, last_name=:last_name, phone=:phone WHERE contact_id=:contact_id";
    try {
        $db = getDB();
        $stmt = $db->prepare($sql);
        $stmt->bindParam("first_name", $w->first_name);
        $stmt->bindParam("last_name", $w->last_name);
        $stmt->bindParam("phone", $w->phone);
        // $stmt->bindParam("region", $w->region);
        // $stmt->bindParam("year", $w->year);
...
function addMycontact() {
    ...
    try {
        ...
        // $stmt->bindParam("description", $wq->description);
        $stmt->execute();
        $wq->id = $db->lastInsertId();
        $db = null;
        echo json_encode($wq);
    } catch(PDOException $e) {
        echo '{"error":{"text":'. $e->getMessage() .'}}';
